function [a,b,c,d,e,f,g,h,i,j,l] = Autoread(mouse, dr, trainPeriod)
% Reads and creates arrays from data sets
% m = mouse : d = directory : p = training period
...
dayIx(trainPeriod).normalHIx = 0;
dayIx(trainPeriod).normalPIx = 0;
...
for k = 1:totalDays
    ...
    dayIx(k).reqHoldTimeIx = (double(input.fixedReqHoldTimeMs) + double(input.randReqHoldMaxMs) + double(input.tooFastTimeMs)) - min(double(cell2mat(input.holdTimesMs)));
    dayIx(k).reactWindowIx = (max(double(cell2mat(input.holdTimesMs))) - (double(input.fixedReqHoldTimeMs) + double(input.randReqHoldMaxMs) + double(input.tooFastTimeMs)));
    ...
    [dayIx(k).normalHIx,dayIx(k).normalPIx] = chi2gof(reactMat, 'Alpha', 0.10);
    ...
    if isempty(normalH)
        normalH = {[dayIx(k).normalHIx]};
    else
        current = length(normalH);
        normalH = vertcat(normalH(1:current,1), dayIx(k).normalHIx);
    end
    %}
    % P value of Normal Chi Square Test
    if isempty(normalP)
        normalP = {[dayIx(k).normalPIx]};
    else
        current = length(normalP);
        normalP = vertcat(normalP(1:current,1), dayIx(k).normalPIx);
    end
end
