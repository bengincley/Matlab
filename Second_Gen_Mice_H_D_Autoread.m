...
out = sprintf('i%s initializing', mouse1);
disp(out)
%Initialize struct
...
i527_scaledVarReactTimeCorrect = {};
i527_scaledVarReactTimeEarly = {};
i527_correctRate = {};
i527_earlyRate = {};
i527_missRate = {};
i527_cdf_pUpper = {};
...
out = sprintf('i%s loading...', mouse1);
disp(out)
...
out = sprintf('i%s loading complete', mouse1);
disp(out)

%Variables for Counting
totalDays = length(dataFiles);

out = sprintf('i%s calculating...', mouse1);
disp(out)
...
for k = 1:totalDays
    ...
    if isempty(i527_medianReactTimeCorrect)
        i527_medianReactTimeCorrect = {[dayIx(k).medianReactTimeCorrectIx]};
    else
        current = length(i527_medianReactTimeCorrect);
        i527_medianReactTimeCorrect = vertcat(i527_medianReactTimeCorrect(1:current,1), dayIx(k).medianReactTimeCorrectIx);
    end
    % Median Reaction Time for Early Trials
    if isempty(i527_medianReactTimeEarly)
        i527_medianReactTimeEarly = {[dayIx(k).medianReactTimeEarlyIx]};
    else
        current = length(i527_medianReactTimeEarly);
        i527_medianReactTimeEarly = vertcat(i527_medianReactTimeEarly(1:current,1), dayIx(k).medianReactTimeEarlyIx);
    end
    % Scaled Variance for Correct Trials
    if isempty(i527_scaledVarReactTimeCorrect)
        i527_scaledVarReactTimeCorrect = {[dayIx(k).scaledVarReactTimeCorrectIx]};
    else
        current = length(i527_scaledVarReactTimeCorrect);
        i527_scaledVarReactTimeCorrect = vertcat(i527_scaledVarReactTimeCorrect(1:current,1), dayIx(k).scaledVarReactTimeCorrectIx);
    end
    % Scaled Variance for Early Trials
    if isempty(i527_scaledVarReactTimeEarly)
        i527_scaledVarReactTimeEarly = {[dayIx(k).scaledVarReactTimeEarlyIx]};
    else
        current = length(i527_scaledVarReactTimeEarly);
        i527_scaledVarReactTimeEarly = vertcat(i527_scaledVarReactTimeEarly(1:current,1), dayIx(k).scaledVarReactTimeEarlyIx);
    end
    % Upper p Value
    if isempty(i527_cdf_pUpper)
        i527_cdf_pUpper = {[dayIx(k).pUpperIx]};
    else
        current = length(i527_cdf_pUpper);
        i527_cdf_pUpper = vertcat(i527_cdf_pUpper(1:current,1), dayIx(k).pUpperIx);
    end
    % Lower p Value
    if isempty(i527_cdf_pLower)
        i527_cdf_pLower = {[dayIx(k).pLowerIx]};
    else
        current = length(i527_cdf_pLower);
        i527_cdf_pLower = vertcat(i527_cdf_pLower(1:current,1), dayIx(k).pLowerIx);
    end
    % Difference p Value
    if isempty(i527_cdfDiff)
        i527_cdfDiff = {[dayIx(k).diffIx]};
    else
        current = length(i527_cdfDiff);
        i527_cdfDiff = vertcat(i527_cdfDiff(1:current,1), dayIx(k).diffIx);
    end
    % Correct Rate
    if isempty(i527_correctRate)
        i527_correctRate = {[dayIx(k).correctRateIx]};
    else
        current = length(i527_correctRate);
        i527_correctRate = vertcat(i527_correctRate(1:current,1), dayIx(k).correctRateIx);
    end
    % Early Rate 
    if isempty(i527_earlyRate)
        i527_earlyRate = {[dayIx(k).earlyRateIx]};
    else
        current = length(i527_earlyRate);
        i527_earlyRate = vertcat(i527_earlyRate(1:current,1), dayIx(k).earlyRateIx);
    end
    % Miss Rate
    if isempty(i527_missRate)
        i527_missRate = {[dayIx(k).missRateIx]};
    else
        current = length(i527_missRate);
        i527_missRate = vertcat(i527_missRate(1:current,1), dayIx(k).missRateIx);
    end
end
out = sprintf('i%s complete', mouse1);
disp(out)
%{
%%%%%%%%%%%
%  i529  
