...
%% Check if ROIs have been selected previously, allow jump straight to figures.   
    ...
    if exist(fn_tc,'file') && exist(fn_mask,'file')
        ...
        if exist(fn_img,'file')
            ...
            out_frames = (g_ind+1):g_ind:sz(3);
            bframes = setdiff(tframes, gframes);
            bframes = setdiff(bframes, out_frames);
            ...
            %save(fn_img, 'input', 'sz', 'stimOn', 'ntrials', 'preframes', 'postframes', 'bdata_trials_gsub', 'bdata_trials_dFoverF', 'gdata_trials_dFoverF', 'gratingDirectionDeg', 'dirs', '-v7.3');
...
%% Plots
    ...
    %             plot(1:size(tc_sub,1), mean(tc_sub(:,ind,1),2), col_mat(id,:));
        ...
        savefig(fullfile(fn_output,[date '_' mouse '_' expt_name '_' num2str(xObj) '_' area_list(ia,:) '_sub.fig']));   
